...
def classify_counterfeit(image_bytes: bytes, session) -> dict:
    ...
    # 3. Retrieve and Normalize Class Metadata (Ensures key safety)
    meta = session.get_modelmeta().custom_metadata_map
    raw_names = ast.literal_eval(meta.get('names', '{}'))
    # Convert any string-formatted keys into integer indexes safely
